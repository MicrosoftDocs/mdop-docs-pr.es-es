...
---
# Introducción a las herramientas de DaRT 8.0


Desde la ventana conjunto de herramientas de diagnóstico **y recuperación** de Microsoft Diagnostics and Recovery Toolset (DaRT) 8,0, puede iniciar cualquiera de las herramientas individuales que incluye al crear la imagen de recuperación de DART 8,0. Para obtener información sobre cómo obtener acceso a la ventana **conjunto de herramientas de diagnóstico y recuperación** , consulte [Cómo recuperar equipos locales con la imagen de recuperación de DART](how-to-recover-local-computers-by-using-the-dart-recovery-image-dart-8.md).
...
A continuación, se ofrece una descripción de las herramientas de DaRT 8,0.
...
Para obtener más información acerca del **analizador de bloqueo**, consulte [diagnosticar errores del sistema con el analizador de bloqueos](diagnosing-system-failures-with-crash-analyzer--dart-8.md).

### Defender

**Importante**  Los entornos con DaRT defender implementado deberían usar en su lugar la imagen de protección sin conexión de Microsoft defender (WDO) para la detección de malware. Debido a la integración de la herramienta de defender en DaRT, todas las implementaciones de la versión de DaRT compatibles no pueden aplicar estas actualizaciones antimalware a sus imágenes de DaRT. Para obtener más información, consulte [Microsoft Diagnostics and Recovery Toolset (DART) los usuarios deben usar Microsoft defender offline (WDO) para la detección de malware: >](use-windows-defender-offline-wdo-for-malware-protection-not-dart.md).

 

**Defender** puede ayudar a detectar malware y software no deseado y avisarle de los riesgos de seguridad. Puede usar esta herramienta para explorar un equipo y quitar malware incluso cuando no se está ejecutando el sistema operativo Windows instalado. Cuando **defender** detecta software malintencionado o no deseado, le preguntará si desea quitar, poner en cuarentena o permitir para cada elemento.
