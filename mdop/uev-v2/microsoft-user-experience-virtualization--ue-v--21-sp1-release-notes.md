...
---
# Notas de la versión de Virtualización de la experiencia del usuario de Microsoft (UE-V) 2.1 SP1


Para buscar las notas de la versión de virtualización de Microsoft User Experience Virtualization 2,1, presione Ctrl + F.
...
Esta sección contiene notas de la versión para la experiencia del usuario de virtualización de 2,1 SP1.
...
Dos cambios realizados en el instalador de UE-V pueden hacer que los scripts de instalación silenciosa que funcionaban en versiones anteriores de UE-V fallaran al instalar UE-V 2,1 SP1. La primera es un nuevo requisito que los usuarios deben aceptar los términos de licencia y aceptar o rechazar la participación en el programa para la mejora de la experiencia del usuario (CEIP), incluso durante una instalación silenciosa. El uso del parámetro/q ya no es suficiente para indicar la aceptación de los términos de licencia y el contrato para participar en el CEIP.
...
SOLUCIÓN: al migrar de UE-V 1 a UE-V 2 y es probable que tenga equipos en los que se ejecuta la versión anterior del agente, cree un catálogo UE-V 2. x independiente para admitir el agente y las plantillas de UE-V 2. x.

### Tiempo de cierre de la UE-V

Ocasionalmente, al cerrar sesión, UE-V tarda mucho tiempo en sincronizar la configuración. Normalmente, esto se debe a una red de latencia alta o a un uso incorrecto del sistema de archivos de distrubuted (DFS).
Para obtener compatibilidad con DFS, consulte [la declaración de soporte de Microsoft sobre los datos de Perfil de usuario replicados](https://support.microsoft.com/kb/2533009) para obtener más información.

SOLUCIÓN alternativa: a partir de HF03, se introdujo una nueva clave del registro la siguiente clave del registro proporciona un mecanismo por el cual se puede especificar el retraso máximo de cierre de sesión \\Software\\Microsoft\\UEV\\Agent\\Configuration\\LogOffWaitInterval

Consulte [la configuración del registro de UE-V](https://support.microsoft.com/kb/2770042) para obtener más información

## Revisiones y artículos de Knowledge base para UE-V 2,1 SP1


Esta sección contiene revisiones y artículos de KB para UE-V 2,1 SP1.
